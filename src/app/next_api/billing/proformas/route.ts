...
const proformasCrud = new CrudOperations('proformas');
const proformaItemsCrud = new CrudOperations('proforma_items');
...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  const data = await proformasCrud.findMany(filters, limit, offset);
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const documentNumber = `PF ${currentYear}/000001`; // Implementar sequência automática
  ...
  const proforma = await proformasCrud.create(proformaData);

  // Criar itens da proforma
  for (const item of body.items) {
    const itemSubtotal = item.quantity * item.unit_price;
    const itemDiscount = (itemSubtotal * (item.discount_percentage || 0)) / 100;
    const itemNetAmount = itemSubtotal - itemDiscount;
    ...
    await proformaItemsCrud.create({
  ...
  return successResponse(proforma, 201);
});
...
export const PUT = withErrorHandling(async (request: NextRequest) => {
  ...
  const existing = await proformasCrud.findById(id);
  if (!existing) {
    return errorResponse('Proforma não encontrada', 404);
  }

  const data = await proformasCrud.update(id, body);
  return successResponse(data);
});
...
export const DELETE = withErrorHandling(async (request: NextRequest) => {
  ...
  const data = await proformasCrud.delete(id);
  return successResponse(data);
});
